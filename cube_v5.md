...
| `TURN_CALIB` | 0.5 | `turn_angle` | 角度修正：转不够**加大**（打滑常见） | 标定转弯 |
...
| `ENC_DRY_TURN_DEG_PER_SEC` | 90.0 | `turn_angle` | DRY_RUN 下按角度估算转动时长 | 无 GPIO 调试 |
...
| `speed` | 转弯 PWM 幅度（正数，默认 50） | 40~55 |
...
| `ENC_SWAP` | 左右脉冲对调 |
| `ENC_DRY_TURN_DEG_PER_SEC` | DRY_RUN 下角速度估算 |
...
| `turn_angle` | 编码器 | `angle_deg`（可选 `speed`,`step`） | `TURN_CALIB`, 轮距, `ENC_*` |
...
|--------|--------|----------------|
| 转弯角度 | `turn_angle(90)` 量角度 | **Config** `TURN_CALIB`, `TRACK_WIDTH_CM`；**传参** `angle_deg` |
...
A: 推荐先用 `turn_angle` 做主转弯（重复性更好）；临时微调或很短促的小角度再用开环 `turn_left` / `turn_right`。`turn_angle` 优先按 `angle_deg` 调，再微调 `step`，最后用 `TURN_CALIB` 消除系统偏差。
