...
魔方绕桩任务的**可拼接函数库（v5 增强版）**。在 `cube_v4.py` 全部能力之上，新增左/右轮搜色、减速靠近、丢目标自动恢复、`drive_distance` / `turn_angle` 编码器闭环。
...
| 按距离/角度走 | 无 | `drive_distance` / `turn_angle`（编码器脉冲闭环） |
...
| 类型 | 在哪里改 | 什么时候改 |
|------|----------|------------|
| **传参** | `main` 里调用函数时 | 每个赛段：距离、角度、速度、停多近 |
| **PidParams** | `PidParams(kp=..., min_delta=...)` | 靠近 PID 手感 |
| **Config (`CFG`)** | 源码 `Config` 或 `CFG.xxx = ...` | 接线、HSV、轮径、CALIB、超时 |

**v5 额外规则：**

- `drive_distance` / `turn_angle` 的**目标距离/角度** → **传参**；**轮径、CALIB、提前停脉冲** → **Config**
- `approach_target_recover` 里恢复用的后退/搜色参数 → **传参**（有默认值，可按赛段改）

---

## 3. v5 专有 Config（编码器距离/角度）

v4 Config 见 [cube_v4.md §4](./cube_v4.md#4-config-总表按用途分类)。下表为 **v5 新增或行为变更** 的字段。

| 字段 | 默认 | 关联 API | 说明 | 何时改 |
|------|------|----------|------|--------|
| `WHEEL_DIAMETER_CM` | 6.5 | `drive_distance`, `forward_pid` | 车轮直径（cm） | 换轮胎 |
| `TRACK_WIDTH_CM` | 14.0 | `turn_angle` | 两轮中心距（cm） | 量车体 |
| `DIST_CALIB` | 0.9 | `drive_distance` | 距离修正：走短了**加大**，走长了减小 | 标定直行 |
| `TURN_CALIB` | 0.9 | `turn_angle` | 角度修正：转不够**加大**（打滑常见） | 标定转弯 |
| `ENC_FINISH_PULSES` | 4 | `drive_distance`, `turn_angle` | 提前停脉冲，吃惯性 | 停点过冲 |
| `ENC_TIMEOUT_S` | 6.0 | `drive_distance`, `turn_angle` | 单动作超时（打滑/卡死） | 动作完不成 |
| `ENC_LOOP_INTERVAL` | 0.005 | `drive_distance`, `turn_angle` | 闭环控制周期（秒） | 极少改 |
| `ENC_SWAP` | False | 编码器读数左右对调 | 与 `SWAP_WHEELS` 不一致时试 True | 脉冲左右反 |
| `ENC_DRY_CM_PER_SEC` | 35.0 | DRY_RUN 模拟速度 | 电脑调试 | 无 GPIO 时 |

直行跑偏优先调各编码器函数的 **`step` 传参**（`forward_pid`、`drive_distance`、`turn_angle`），无需额外 Config 补偿项。
...
from cube_v5 import (
    ...
    drive_distance, turn_angle, forward_pid, PidParams,
)
...
## 10. drive_distance(dist_cm, speed, step) 【v5 新增】

**功能：** 编码器脉冲闭环直行指定**厘米数**；左右用 `step` 离散同步修正（同 `forward_pid` 思路）。执行完 `stop()`。

```python
drive_distance(30, 40, 0.2)    # 前进约 30 cm
...
| 参数 | 说明 | 典型值 |
|------|------|--------|
| `dist_cm` | 距离（cm）；**正=前进，负=后退** | 10~50 按赛道 |
...
|--|------------------|---------------|
| 停止条件 | 累计脉冲达目标 | **时间** `duration` 到 |
| 传参重点 | `dist_cm`, `speed`, `step` | `duration`, `speed`, `step` |
| Config 重点 | `DIST_CALIB`, 轮径 | 编码器引脚 |

---

## 11. turn_angle(angle_deg, speed, step) 【v5 新增】
...
| `ENC_LOOP_INTERVAL` | 控制周期 |
| `ENC_SWAP` | 左右脉冲对调 |
| 编码器引脚 / `ENCODER_PULSES_PER_REV` | 同 `drive_distance` |

可替代 `turn_left(duration, wheel_speed)`；重复性更好，推荐用传参 `angle_deg` 而非猜时间。

---

## 12. 开环 / 按时间编码器 API（与 v4 相同）
...
| `forward_pid` | `left/right_speed`, `step`, `interval`, `duration` | 编码器引脚、`FORWARD_PID_INTERVAL` |
...
| `drive_distance` | 编码器 | `dist_cm`, `speed`, `step` | `DIST_CALIB`, 轮径, `ENC_*` |
...
可将 `turn_left` + `forward_pid` 换成 `turn_angle` + `drive_distance`；将 `approach_target` 换成 `approach_target_recover` / `approach_target_brake`：
...
|--------|--------|----------------|
| 直行距离 | `drive_distance(30,40,0.2)` 量实际 cm | **Config** `DIST_CALIB`, `WHEEL_DIAMETER_CM`；**传参** `dist_cm` |
| 转弯角度 | `turn_angle(90,50,0.2)` 量角度 | **Config** `TURN_CALIB`, `TRACK_WIDTH_CM`；**传参** `angle_deg` |
| 到位过冲 | 停点后仍滑 | **Config** `ENC_FINISH_PULSES` |
| 直行仍漂 | 长距离弯一侧 | **传参** `step`（`forward_pid` / `drive_distance` / `turn_angle`） |
...
**Q: `drive_distance` 和 `forward_pid` 该用哪个？**  
A: 知道厘米数 → `drive_distance`；只有时间标定（如冲线 3s）→ `forward_pid`。
